#############################################################################################################
#
# File train model dựa trên data csv, json
#
#############################################################################################################
...
class CollabFiltering(object):
    ...
    #   uuCF: Sử dụng tương đồng user-user là 1, sử dụng item-item là 0. Mặc định sử dụng user-user collaborative filtering
        ...
        # user_id_col = self.Y_data[:, 0].tolist()
        # post_id_col = self.Y_data[:, 2].tolist()
        # # Tạo 1 set các user_id, post_id khác nhau
        # self.set_userID = set(user_id_col)
        # self.set_postID = set(post_id_col)
        # # + 1 do mảng bắt đầu từ 0
        # self.n_users = len(self.set_userID) + 1
        # self.n_items = len(self.set_postID) + 1
    ...
    def normalize_matrix(self):
        ...
        for i in range(self.n_users):
            ...
            tmp = np.mean(ratings)
            if np.isnan(tmp):
                tmp = 0  # để tránh mảng trống và nan value
            self.avg_rating[i] = tmp
            # chuẩn hóa
            self.Ybar_data[ids, 2] = ratings - self.avg_rating[i]
